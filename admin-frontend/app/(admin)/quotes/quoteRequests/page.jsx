...
export default function AdminQuoteRequestsPage() {
  ...
  // Delete loading state (per quote)
  const [deletingId, setDeletingId] = useState(null);

  // Custom Service Modal
  const [showCustomServiceModal, setShowCustomServiceModal] = useState(false);
  const [customServiceMode, setCustomServiceMode] = useState("create"); // "create" or "edit"
  ...
  // Add these
  const [showCustomServiceViewModal, setShowCustomServiceViewModal] =
    useState(false);
  const [viewingCustomService, setViewingCustomService] = useState(null);
  ...
  const handleCreateCustomService = async (quoteId) => {
    if (!window.confirm("Create custom service from this quote request?"))
      return;

    try {
      const res = await apiFetch(
        `/api/bookings/admin/quote-requests/${quoteId}/create-custom-service/`,
        { method: "POST" },
      );
  ...
  const handleEditQuote = (quote) => {
    router.push(`/quotes/quoteRequests/edit/${quote.id}`);
    setOpenDropdownId(null);
  };
  ...
  // New function to open modal with prefilled data
  const openCustomServiceModal = async () => {
    ...
    if (isEdit) {
      ...
      setCustomServiceForm({
        ...
        whats_included: JSON.stringify(data.whats_included || []),
  ...
  const openCustomServiceView = async (customServiceId) => {
    setViewLoading(true);
    setShowCustomServiceViewModal(true);

    try {
      const data = await apiFetch(
        `/api/bookings/admin/custom-services/${customServiceId}/`,
      );
      setViewingCustomService(data);
    } catch (err) {
      alert("Failed to load custom service details");
      setShowCustomServiceViewModal(false);
    } finally {
  ...
  const handleCustomServiceSubmit = async (e) => {
    ...
    const raw = customServiceForm.whats_included.trim();
    ...
    if (raw) {
      try {
        ...
        if (Array.isArray(parsed)) {
          whatsIncludedArray = parsed;
        } else {
          throw new Error();
        }
      } catch {
        // Fallback: comma separated
        whatsIncludedArray = raw
          .split(",")
          .map((i) => i.trim())
          .filter(Boolean);
      }
    ...
    const payload = {
      quote_request: selectedQuote.id,
  ...
  const handleDeleteCustomService = async (customServiceId) => {
    if (!window.confirm("Delete this custom service?\nThis cannot be undone."))
      return;

    setCustomServiceLoading(true); // reuse loading state for simplicity
    ...
    try {
      ...
      alert("Custom service deleted successfully!");
      fetchQuoteDetail(selectedQuote.id); // refresh sidebar
      fetchQuotes(); // refresh list to update indicators
    } catch (err) {
      alert(err?.detail || "Failed to delete custom service");
    } finally {
      setCustomServiceLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-7 pb-10">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-5">
        ...
        <Link
          href="/quotes/quoteRequests/create"
                        ...
                        <div className="relative flex justify-end">
                          ...
                          {isOpen && !isDeleting && (
                            ...
                            >
                              ...
                              {/* Custom Service Buttons in Dropdown */}
                              {!q.custom_service ? (
                                <button
                                  onClick={() => {
                                    setSelectedQuote(q); // set for modal prefill
                                    openCustomServiceModal();
                                    setOpenDropdownId(null);
                                  }}
                                  className="flex w-full items-center gap-3 px-5 py-3 hover:bg-blue-50 text-blue-700 text-sm"
                                >
                                  <Send className="w-4 h-4" /> Create Custom
                                  Service
                                </button>
                              ) : (
                                <>
                                  <button
                                    ...
                                    className="flex w-full items-center gap-3 px-5 py-3 hover:bg-blue-50 text-blue-700 text-sm"
                                  >
                                    <Edit className="w-4 h-4" /> Edit Custom
                                    Service
                                  </button>
                                  ...
                                  <button
                                    ...
                                    className="flex w-full items-center gap-3 px-5 py-3 hover:bg-red-50 text-red-600 text-sm"
                                    disabled={customServiceLoading}
                                  >
                                    <Trash2 className="w-4 h-4" /> Delete Custom
                                    Service
                                  </button>
                                </>
                              )}
                      ...
                      <>
                        {!selectedQuote.custom_service ? (
                          <button
                            ...
                            disabled={customServiceLoading}
                            className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl transition flex items-center gap-2 shadow-sm disabled:opacity-60"
                          >
                          ...
                          <>
                            ...
                            <button
                              ...
                              disabled={customServiceLoading}
                              className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-medium rounded-xl transition flex items-center gap-2 shadow-sm disabled:opacity-60"
                            >
                              {customServiceLoading && (
                                <Loader2 className="w-4 h-4 animate-spin" />
                              )}
        ...
        >
          ...
          >
            <img
              ...
              className="max-w-full max-h-[85vh] object-contain transition-transform duration-200 hover:scale-110 cursor-zoom-in"
            />
          </div>
        </div>
      )}
      ...
      {/* Custom Service View Modal */}
      {showCustomServiceViewModal && (
        <div className="fixed inset-0 bg-black/60 z-[70] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b p-6 flex justify-between items-center">
              ...
              <button
                onClick={() => setShowCustomServiceViewModal(false)}
                className="p-2 hover:bg-slate-100 rounded-lg"
              >
                <X className="w-6 h-6 text-slate-500" />
              </button>
            ...
            <div className="p-6 space-y-6">
              ...
              ) : viewingCustomService ? (
                <div className="space-y-6">
                  {/* Header Info */}
                  <div className="bg-orange-50 p-5 rounded-xl">
                    <h3 className="text-xl font-bold text-slate-900 mb-2">
                      {viewingCustomService.name}
                    </h3>
                    <p className="text-sm text-slate-600">
                      ID: {viewingCustomService.id} • Created:{" "}
                      {formatDate(viewingCustomService.created_at)}
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <p className="mt-1 text-slate-800">
                        {viewingCustomService.description || "—"}
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-slate-800">
                        ₹{viewingCustomService.base_price} (
                        {viewingCustomService.price_unit})
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-slate-800">
                        {viewingCustomService.duration_minutes} minutes
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-slate-800">
                        {viewingCustomService.warranty_days} days
                      </p>
                    </div>
                    ...
                    <div className="md:col-span-2">
                      ...
                      <ul className="mt-2 list-disc pl-5 text-slate-700">
                        {viewingCustomService.whats_included?.length > 0 ? (
                          viewingCustomService.whats_included.map(
                    ...
                    <div>
                      ...
                      <span
                        className={`inline-flex mt-1 px-3 py-1 rounded-full text-xs font-semibold ${
                          getStatusConfig(viewingCustomService.status).badge
                        }`}
                      >
                        {getStatusConfig(viewingCustomService.status).label}
                      </span>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="mt-1">
                        {viewingCustomService.is_active ? (
                  ...
                  <div className="flex justify-end gap-4 pt-6 border-t">
                    <button
                      onClick={() => {
                        setShowCustomServiceViewModal(false);
                        openCustomServiceModal(); // Open your existing edit modal
                      }}
                      className="px-6 py-3 border border-orange-300 hover:bg-orange-50 text-orange-700 font-medium rounded-xl transition flex items-center gap-2"
                    >
                      <Edit className="w-4 h-4" />
                      Edit
                    </button>
                    ...
                    <button
                      onClick={() => {
                        if (window.confirm("Delete this custom service?")) {
                          handleDeleteCustomService(viewingCustomService.id);
                          setShowCustomServiceViewModal(false);
                        }
                      }}
                      className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white font-medium rounded-xl transition flex items-center gap-2 shadow-sm"
                    >
                      <Trash2 className="w-4 h-4" />
                      Delete
                    </button>
